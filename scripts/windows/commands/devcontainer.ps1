...
.DESCRIPTION
Idempotent script. Builds the base image and the devcontainer image (with
--no-cache if -Force is set), then starts an interactive shell.
The Docker socket is mounted for sibling container operations.

.PARAMETER Force
Force rebuild of the devcontainer image with --no-cache.
